...
Definition error13: string := "data conflict". 

Definition error14: string := "control conflict". 

Definition error15: string := "duplicated pending events".

Definition error16: string := "variable not defined".
...
Fixpoint checkDuplicatedElement (A:Type) (l: list A) (decA: forall (x y:A), {x=y} + {x<>y}): bool :=
match l with
| nil => false
| e'::l' => if inList decA e' l' then true else checkDuplicatedElement l' decA
end.

Lemma NoDupUniq: forall (A:Type) l decA,
 checkDuplicatedElement l decA = false <-> @NoDup A l.
Proof. intros A l. split.
- induction l. 
+ intros. constructor.
+ intros. constructor. simpl in H. 
remember (inList decA a l ). destruct b. inversion H.
SearchAbout inList. 
apply reverseinListFalse with (decA:=decA). symmetry;auto.
...
*
simpl.     
remember (inList decA x l ).
destruct b. 
 SearchAbout inList. symmetry in Heqb.  apply reverseinListLemma in Heqb. exfalso;apply H;auto.
auto.
Qed.  
